...
module.exports = class MoonbeamDevNode extends EventEmitter {
  constructor(opts) {
    ...
    this.process.stdout?.on("data", chunk => {
      for (var line of stdout.push(chunk)) this.emit("log", line, "stdout")
    })

    this.process.stderr?.on("data", chunk => {
      for (var line of stderr.push(chunk)) this.emit("log", line, "stderr")
    })
  }
  ...
  hasStarted() {
    return Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(Error("connection timeout after" + this.maxStartMs)),
          this.maxStartMs
        )
      )
    ])
